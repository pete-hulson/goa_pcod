...
LENGTH4AGE_BY_CATCH_GOA<-function(fsh_sp_str=202 ,fsh_sp_label = "'PCOD'",ly=2019, STATE=3,max_age=10){
 	...
 	test <- paste("SELECT \n ",
  	...
  	vroom::vroom_write(Dspcomp, here::here(ly, 'data_old', 'raw', 'fish_agecomp.csv'), delim = ",")
    ...
    vroom::vroom_write(CATCH, here::here(ly, 'data_old', 'raw', 'catch4fish_agecomp.csv'), delim = ",")
  ...
  t3[,'TFREQ':=NULL]
    ...
    SLENGTH<-read.csv(here::here(ly, "data_old", "ALL_STATE_LENGTHS.csv"))
