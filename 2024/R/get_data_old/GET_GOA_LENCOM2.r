GET_GOA_LENCOMP2<-function(fsh_sp_str1=202, len_bins1=len_bins, fsh_start_yr1=fsh_start_yr, new_SS_dat_year1= new_SS_dat_year, seas=1,gender=1,part=0,Nsamp=-1) {
  ...
  vroom::vroom_write(Fspcomp, here::here(new_SS_dat_year1, 'data_old', 'raw', 'fish_lencomp_foreign.csv'), delim = ",")
  Fspcomp$GEAR1<-"TRAWL"
...
## proportion lengths for each gear type based on haul size (extrapolated number of fish in haul) 
 ...
 test <- paste("SELECT \n ",
  ...
  vroom::vroom_write(Dspcomp, here::here(new_SS_dat_year1, 'data_old', 'raw', 'fish_lencomp_domestic.csv'), delim = ",")
  Dspcomp$GEAR1<-"TRAWL"
