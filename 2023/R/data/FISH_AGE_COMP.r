...
LENGTH4AGE_BY_CATCH_GOA<-function(fsh_sp_str=202 ,fsh_sp_label = "'PCOD'",ly=2019, STATE=3,max_age=10){
 	...
 	test <- paste("SELECT \n ",
  	...
  	vroom::vroom_write(Dspcomp, here::here(new_year, 'data', 'raw', 'fish_agecomp.csv'), delim = ",")
    ...
    vroom::vroom_write(CATCH, here::here(new_year, 'data', 'raw', 'catch4fish_agecomp.csv'), delim = ",")
