...
dir<-"C:/WORKING_FOLDER/2021 Stock Assessments/2021 Pacific cod/Models/Model21.1e"

replot_profiles<-function(profile=dir,CYR=2021,SYR=1977,FCASTY=15){
	...
	SSB_unfished<-output$SSB$SSB_unfished[1]
		...
		summ2<-rbind(y,y1,y2,profile$SSB)
		...
		C_ABC<-data.table(mods1[[1]]$derived_quants)[Label=="Dead_Catch_Btgt"]
		C_OFL<-data.table(mods1[[6]]$derived_quants)[Label=="Dead_Catch_Btgt"]
		
		y<-data.table(Yr=c(CYR+1:EYR),Catch=C_ABC,Catch_std=0,model="Catch Fmaxabc")
		y1<-data.table(Yr=c(CYR+1:EYR),Catch=C_OFL,Catch_std=0,model="Catch Fofl")
		Pcatch2<-rbind(y,y1,profile$CATCH)
