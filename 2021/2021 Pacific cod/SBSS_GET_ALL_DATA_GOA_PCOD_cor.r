...
# currently all functions are hardcoded for 4 fishing fleets and 1 survey -> 5 total
 ...
 #                               len_bins           = len_bins
 #                               max_age            = max_age
 #                               use_srv_slope_data = use_srv_slope_data
 #                               use_srv_ll_data    = use_srv_ll_data
 #                               use_srv_AI_data    = use_srv_AI_data
...
SBSS_GET_ALL_DATA <- function(new_data     = new_data,
                              ...
                              max_age      = 10,
                              use_srv_slope_data = FALSE,
                              use_srv_ll_data    = FALSE,
                              use_srv_AI_data    = FALSE,
...
## format catch for SS3
# catch<-data.frame(TRAWL=CATCH$TONS[CATCH$GEAR=="TRAWL"],LONGLINE=CATCH$TONS[CATCH$GEAR=="LONGLINE"],POT=CATCH$TONS[CATCH$GEAR=="POT"],year=CATCH$YEAR[CATCH$GEAR=="TRAWL"],seas=rep(1,length(CATCH$YEAR[CATCH$GEAR=="TRAWL"])))
#
...
## pulling in LL survey RPN data from Dana's csv files.
 LL_RPN<-GET_GOA_LL_RPN(species=21720,FYR=1977)
 LL_RPN<-LL_RPN[year>=LLsrv_start_yr]
 LL_CPUE<-data.frame(year=as.numeric(LL_RPN[1,2:ncol(LL_RPN)]),seas=7,index=5,obs=as.numeric(LL_RPN[2,2:ncol(LL_RPN)])*1000,se_log=as.numeric(LL_RPN[3,2:ncol(LL_RPN)])/as.numeric(LL_RPN[2,2:ncol(LL_RPN)]))
 CPUE<-rbind(CPUE,LL_CPUE)

  if(exists("ADFG_IPHC")){ 
       names(ADFG_IPHC)<-names(CPUE)
       CPUE<-rbind(CPUE,ADFG_IPHC)
     }else {print("no ADFG_IPHC file appears to exist here")}
...
## ----- Get Fishery catch at size composition data -----
 ...
 FISHLCOMP<-data.frame(GET_GOA_LENCOMP2(fsh_sp_str1=202, len_bins1=len_bins, fsh_start_yr1=fsh_start_yr, new_SS_dat_year1= new_year, seas=7,gender=0,part=0,Nsamp=-1)) 
...
## format survey mean size-at-age data for SS3
  AGE_LENGTH_SS <- data.frame(FORMAT_AGE_MEANS1(srv_age_samples=Age_w,max_age=max_age,type="L",seas=7,flt=-4,gender=0,part=0))
