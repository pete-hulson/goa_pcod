...
dir1<-"C:/Users/steve.barbeaux/Work/WORKING_FOLDER/climate_data/Heatwave_Files/data2"
...
#
#####################################################################################################
dir1<-"C:/Users/steve.barbeaux/Work/WORKING_FOLDER/climate_data/data2"


setwd(dir1)
#load('Heatwave_figure.RData')
load('Heatwave_figure11.15.25.RData')

file=list.files(pattern=".nc")
file.remove(file)

## Going to coastwatch 
#download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2022-09-11T12:00:00Z):1:(2023-06-11T12:00:00Z)%5D%5B(0.0):1:(0.0)%5D%5B(52):1:(62)%5D%5B(200):1:(215)%5D"),method = "libcurl", mode="wb",destfile = "test_OISST.nc")
#download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2023-06-12T12:00:00Z):1:(2024-06-11T12:00:00Z)%5D%5B(0.0):1:(0.0)%5D%5B(52):1:(62)%5D%5B(200):1:(215)%5D"),method = "libcurl", mode="wb",destfile = "test_OISST2.nc")
#download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2024-06-12T12:00:00Z):1:(2025-06-11T12:00:00Z)%5D%5B(0.0):1:(0.0)%5D%5B(52):1:(62)%5D%5B(200):1:(215)%5D"),method = "libcurl", mode="wb",destfile = "test_OISST3.nc")
#download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2025-06-12T12:00:00Z):1:(last)%5D%5B(0.0):1:(0.0)%5D%5B(52):1:(62)%5D%5B(200):1:(215)%5D"),method = "libcurl", mode="wb",destfile = "test_OISST4.nc")
download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2025-11-17T12:00:00Z):1:(last)%5D%5B(0.0):1:(0.0)%5D%5B(52):1:(62)%5D%5B(200):1:(215)%5D"),method = "libcurl", mode="wb",destfile = "test_OISST.nc")

#download.file(url = paste0("https://coastwatch.pfeg.noaa.gov/erddap/griddap/ncdcOisst21NrtAgg.nc?sst[(2022-09-11T12:00:00Z):1:(last)][(0.0):1:(0.0)][(52):1:(62)][(200):1:(215)]"),
#            method = "libcurl", mode="wb",destfile = "test_OISST.nc")
#download.file(url = paste0("https://www.ncei.noaa.gov/erddap/griddap/ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon.nc?sst%5B(2022-09-26T12:00:00Z):1:(last)%5D%5B(0.0):1:(0.0)%5D%5B(52.000):1:(62.000)%5D%5B(200.000):1:(215.000)%5D"),
#              method = "libcurl", mode="wb",destfile = "test_OISST.nc")
...
hobday<-data.table(hobday)[order(day1)]
...
d<-d+labs(subtitle="Previous 90 days")+theme(plot.subtitle=element_text(vjust=-2,hjust=0.5,size=15))
...
d2<-d2+scale_x_date(breaks = function(x) seq.Date(from = as_date("1982-01-01",format="%Y-%m-%d",tz="PST"), to = as_date("2026-12-01",format="%Y-%m-%d",tz="PST"), by = "2 years"),labels = date_format("%Y"))
...
d2<-d2+labs(subtitle="1981 - Present")+theme(plot.subtitle=element_text(vjust=-2,hjust=0.6,size=15))
...
png("MHWI.png",width=9,height=4,units="in",res=300)
print(d)
dev.off()
...
x2<-hobday[MONTH%in% c(1:5)&YEAR%in%c(2015:2017)][,list(MEANSST=mean(SST)),by="doy"]
dsp<-ggplot(hobday[MONTH%in%c(1:5)&YEAR<2026],aes(x=doy,y=SST,group=YEAR))+geom_line(color='gray80',size=0.5)+theme_bw(base_size=18)+geom_line(data=x,aes(x=doy,y=MEANSST,group=NA),color="gray50",size=1.25,linetype=2)+geom_line(data=x2,aes(x=doy,y=MEANSST,group=NA),color="red",size=1.25,linetype=2)
dsp<-dsp+geom_hline(yintercept=5,color="black",linetype=3)+
#geom_line(data=hobday[MONTH%in%c(1:5)&YEAR==2022],size=1.25,color="black")+
geom_line(data=hobday[MONTH%in%c(1:5)&YEAR>2017&YEAR<2026],aes(color=factor(YEAR)),size=1)+scale_color_manual(values=c(2:8,'black'))
dsp<-dsp+labs(y=expression('Sea surface temperature ('~degree*C~")"),x="Day of the year",color='Year', title="January through May 1981-2025")
...
xsu2<-hobday[MONTH%in% c(6:9)&YEAR%in%c(2015:2017)][,list(MEANSST=mean(SST)),by="doy"]
dsu<-ggplot(hobday[MONTH%in%c(6:9)&YEAR<2026],aes(x=doy,y=SST,group=YEAR))+geom_line(color='gray80',size=0.5)+theme_bw(base_size=18)+geom_line(data=xsu,aes(x=doy,y=MEANSST,group=NA),color="gray50",size=1.25,linetype=2)+geom_line(data=xsu2,aes(x=doy,y=MEANSST,group=NA),color="red",size=1.25,linetype=2)
dsu<-dsu+geom_hline(yintercept=5,color="black",linetype=3)+
#geom_line(data=hobday[MONTH%in%c(6:9)&YEAR==2022],size=1.25,color="black")+
geom_line(data=hobday[MONTH%in%c(6:9)&YEAR>2017&YEAR<2026],aes(color=factor(YEAR)),size=1)+scale_color_manual(values=c(2:8,'black'))
dsu<-dsu+labs(y=expression('Sea surface temperature ('~degree*C~")"),x="Day of the year",color='Year', title="June through September 1981-2025")


xfa<-hobday[MONTH%in% c(10:12)&YEAR%in%c(1982:2012)][,list(MEANSST=mean(SST)),by="doy"]
xfa2<-hobday[MONTH%in% c(10:12)&YEAR%in%c(2015:2017)][,list(MEANSST=mean(SST)),by="doy"]
dfa<-ggplot(hobday[MONTH%in%c(10:12)&YEAR<2026],aes(x=doy,y=SST,group=YEAR))+geom_line(color='gray80',size=0.5)+theme_bw(base_size=18)+geom_line(data=xfa,aes(x=doy,y=MEANSST,group=NA),color="gray50",size=1.25,linetype=2)+geom_line(data=xfa2,aes(x=doy,y=MEANSST,group=NA),color="red",size=1.25,linetype=2)
dfa<-dfa+geom_hline(yintercept=5,color="black",linetype=3)+
#geom_line(data=hobday[MONTH%in%c(10:12)&YEAR==2022],size=1.25,color="black")+
geom_line(data=hobday[MONTH%in%c(10:12)&YEAR>2017&YEAR<2026],aes(color=factor(YEAR)),size=1)+scale_color_manual(values=c(2:8,'black'))
dfa<-dfa+labs(y=expression('Sea surface temperature ('~degree*C~")"),x="Day of the year",color='Year', title="October through December 1981-2025")

png("TEMPS.png",width=20,height=10,units="in",res=300)
