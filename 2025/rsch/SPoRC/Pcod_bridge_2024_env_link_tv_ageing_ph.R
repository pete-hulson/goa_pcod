...
for(i in lens_fleet4_yrs) {
  ObsSrvLenComps[1,which(years == i),,1,1] <- unlist((lens_fleet4 %>% dplyr::filter(year == i))[,-c(1:6)]) # observed lens
  UseSrvLenComps[1,which(years == i),1] <- 1 # whether to use fishery lens
  ISS_SrvLenComps[1,which(years == i),1,1] <- (lens_fleet4 %>% dplyr::filter(year == i))$Nsamp
}
...
for(i in lens_fleet5_yrs) {
  ObsSrvLenComps[1,which(years == i),,1,2] <- unlist((lens_fleet5 %>% dplyr::filter(year == i))[,-c(1:6)]) # observed lens
  UseSrvLenComps[1,which(years == i),2] <- 1 # whether to use fishery lens
  ISS_SrvLenComps[1,which(years == i),1,2] <- (lens_fleet5 %>% dplyr::filter(year == i))$Nsamp
}
...
input_list <- Setup_Mod_Fishsel_and_Q(
  ...
  # ),
  # test no blocks
  fish_sel_blocks = c("none_Fleet_1", # longline
                      "none_Fleet_2", # trawl
                      "none_Fleet_3" # pot
  ),
...
parameters$ln_fish_fixed_sel_pars[,2,,,1:2] <- log(2) # slope parameter for logistic, fleet 1 and 2
...
parameters$ln_srv_fixed_sel_pars[,1,,,1] <- log(70) # l50 parameter for logistic, fleet 1
parameters$ln_srv_fixed_sel_pars[,2,,,1] <- log(3) # slope for logistic fleet 1
parameters$ln_srv_fixed_sel_pars[,1,,,2] <- log(70) # lmax parameter for gamma, fleet 1
parameters$ln_srv_fixed_sel_pars[,2,,,2] <- log(10) # slope for gamma fleet 1
