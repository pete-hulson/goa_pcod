...
# explore data
  ...
  tidytable::filter(trip_target_code == 'C',
                    reporting_area_code %in% c(610, 620)) %>% 
  ...
  tidytable::mutate(catch_spawn = sum(catch))
  tidytable::summarise(catch = sum(weight_posted), .by = c(fmp_gear, reporting_area_code, stanza, season)) %>% 
  print(n=200)
